 ...
 *  -----------------------------------------------------------------------------
 *  ★データソース★
 *  【商品コスト指標】※基準 2021年4月=100
 *    小麦  : 農林水産省「輸入小麦の政府売渡価格」(公表値)
 *    原油  : 日銀CGPI「石油・石炭製品」(類別) ✅一次CSV実測
 *    砂糖  : 日銀CGPI「粗糖」輸入物価指数・円ベース ✅一次CSV実測 (ICE依存を解消)
 *    カカオ : 日銀CGPI「コーヒー豆・カカオ豆」輸入物価指数・円ベース ✅一次CSV実測
 *    生乳  : 日銀CGPI「原乳」(品目) ✅一次CSV実測
 *  【仕入れコスト指標】
...
window.CARTEL_DATA = {
  meta: {
    version: "0.6.0",
    updated: "2026-07-06",  // 観測期間 2026-04 まで11時点 / 砂糖・カカオ・生乳も日銀CGPI実測へ移行
    disclaimer:
    ...
    provenance: {
      ...
      commodities: { confidence: "verified", source: "5/6系列（crude/pulp/sugar/cacao/milk）を日銀CGPI一次CSVから機械取得（sugar/cacaoは輸入物価指数・円ベース、ICE先物依存を解消）。wheat のみ農水省売渡価格の公表値" },
  ...
  commodities: [
    ...
    },
    {
      ...
      series: [
        ...
        { date: "2025-04", index: 136.2 }, { date: "2025-10", index: 126.5 },
        { date: "2026-04", index: 143.5 },
      ],
    },
    {
      id: "sugar",
      // ✅検証済み: 日銀CGPI「粗糖」輸入物価指数(円ベース) 一次CSVから機械取得 (2021-04=100)
      // 2023-10: インド輸出規制・エルニーニョ+円安で 210.7 のピーク → 以降反落
      // ICE先物×為替の推定から移行（T016 ライセンス恒久対応・輸入実勢価格で経済的にも正確）
      name: "粗糖 輸入物価指数（円ベース・日銀CGPI実測）",
      series: [
        { date: "2021-04", index: 100.0 }, { date: "2021-10", index: 124.5 },
        { date: "2022-04", index: 142.8 }, { date: "2022-10", index: 165.1 },
        { date: "2023-04", index: 166.4 }, { date: "2023-10", index: 210.7 },
        { date: "2024-04", index: 189.6 }, { date: "2024-10", index: 170.1 },
        { date: "2025-04", index: 163.0 }, { date: "2025-10", index: 141.3 },
        { date: "2026-04", index: 136.7 },
      ],
    },
    {
      id: "cacao",
      // ✅検証済み: 日銀CGPI「コーヒー豆・カカオ豆」輸入物価指数(円ベース) 一次CSVから機械取得
      // 日銀の品目粒度の制約でコーヒー豆と合算（両者とも2024-26年に歴史的高騰）
      // 輸入実勢は先物より遅行して契約に反映され、2026-04 に 413.8 で最高値を更新中
      name: "コーヒー豆・カカオ豆 輸入物価指数（円ベース・日銀CGPI実測）",
      series: [
        { date: "2021-04", index: 100.0 }, { date: "2021-10", index: 141.2 },
        { date: "2022-04", index: 211.7 }, { date: "2022-10", index: 229.1 },
        { date: "2023-04", index: 164.9 }, { date: "2023-10", index: 201.2 },
        { date: "2024-04", index: 244.6 }, { date: "2024-10", index: 320.8 },
        { date: "2025-04", index: 410.2 }, { date: "2025-10", index: 357.0 },
        { date: "2026-04", index: 413.8 },
      ],
    },
    {
      id: "milk",
      // ✅検証済み: 日銀CGPI「原乳」(国内企業物価・品目) 一次CSVから機械取得 (2021-04=100)
      // 2022-11 乳価改定が 2023-04 観測点に反映(+9pt)。農水省公表値ベースの旧推定(122)と整合
      name: "原乳 企業物価指数（日銀CGPI実測）",
      series: [
        { date: "2021-04", index: 100.0 }, { date: "2021-10", index: 100.7 },
        { date: "2022-04", index: 100.0 }, { date: "2022-10", index: 100.7 },
        { date: "2023-04", index: 109.9 }, { date: "2023-10", index: 115.7 },
        { date: "2024-04", index: 117.1 }, { date: "2024-10", index: 117.7 },
        { date: "2025-04", index: 116.5 }, { date: "2025-10", index: 122.3 },
        { date: "2026-04", index: 121.9 },
      ],
    },
    {
      id: "pulp",
      ...
      series: [
        ...
        { date: "2025-04", index: 126.3 }, { date: "2025-10", index: 129.0 },
        { date: "2026-04", index: 132.0 },
      ],
    },
  ],
  ...
  procurement: {
    ...
    inputs: [
      ...
      { id:"rice",      name:"米",            series:[100.0,97.7,94.4,95.9,97.1,100.8,104.7,160.2,207.8,224.5,209.1] },
      // 野菜: 総務省CPI「生鮮野菜」季節変動大
      { id:"vegetable", name:"野菜",          series:[100.0,107.9,112.2,115.1,113.5,136.5,128.8,139.3,132.0,140.3,129.2] },
      // 食肉: 総務省CPI「生鮮肉」
      { id:"meat",      name:"食肉",          series:[100.0,101.7,102.5,107.7,112.1,113.4,114.1,119.1,120.0,124.0,126.2] },
      // 食用油: 総務省CPI「食用油」2022年に大幅高騰
      { id:"oil",       name:"食用油",        series:[100.0,119.6,136.5,162.3,165.1,158.6,149.9,148.8,149.4,152.2,157.6] },
      // 小麦粉: 日銀PPI「小麦粉」(農水省売渡価格連動)
      { id:"flour",     name:"小麦粉",        series:[100.0,106.2,115.2,125.5,125.6,130.9,128.9,134.2,140.6,139.0,137.7] },
      // 洗剤: 総務省CPI「洗濯用洗剤」
      { id:"detergent", name:"洗剤・溶剤",    series:[100.0,98.5,99.9,106.5,119.7,119.0,120.0,137.6,136.8,157.1,155.0] },
      // 電気・ガス: 総務省CPI 電気代+ガス代 (政府補助金期間含む)
      { id:"energy",    name:"光熱費(電気・ガス)", series:[100.0,107.0,119.8,129.1,114.5,110.4,111.9,114.6,123.2,117.4,119.7] },
      // 水道: 総務省CPI「上下水道」
      { id:"water",     name:"水道",          series:[100.0,100.7,101.0,97.2,101.1,100.4,101.6,103.0,103.2,102.6,102.3] },
      // 人件費: 厚労省「地域別最低賃金」全国加重平均の制度実値
      // 902円(R2)→930(R3-10)→961(R4-10)→1004(R5-10)→1055(R6-10)→1121(R7-10) / 902=100
      { id:"labor",     name:"人件費",        series:[100,103.1,103.1,106.5,106.5,111.3,111.3,117.0,117.0,124.3,124.3] },
      // 家賃: 総務省CPI「家賃」(緩やかな上昇)
      { id:"rent",      name:"家賃",          series:[100.0,100.0,100.0,100.1,100.1,100.2,100.3,100.4,100.6,100.9,101.0] },
      // 包装・物流: 日銀PPI「段ボール箱」+「道路貨物輸送」
      { id:"packaging", name:"包装・物流",    series:[100.0,99.7,102.1,108.7,114.8,115.3,116.3,116.5,118.9,121.5,124.0] },
      // その他: CPI諸雑費（推定）
      { id:"misc",      name:"その他",        series:[100,103,106,108,110,112,114,116,118,120,122] },
      // 燃料: 総務省CPI「ガソリン」(2022年高騰, 補助金で抑制後2023再上昇)
      { id:"fuel",      name:"燃料(軽油・ガソリン)", series:[100.0,108.6,115.7,111.7,111.9,117.3,116.9,116.9,124.5,116.9,112.5] },
      // 車両: 総務省CPI「自動車関連費用」
      { id:"vehicle",   name:"車両・整備",    series:[100.0,102.1,103.5,103.3,103.3,105.6,106.9,107.5,110.8,110.5,110.0] },
      // 建材: 国土交通省「建設工事費デフレーター」資材費
      { id:"material",  name:"建材・資材",    series:[100.0,125.4,133.5,139.2,133.0,132.6,133.2,132.9,132.2,133.5,135.6] },
      // 商品仕入れ: 日銀PPI「食料品」卸売物価
      { id:"goods",     name:"商品仕入れ(卸)", series:[100.0,101.3,104.0,108.9,113.1,115.0,116.0,117.4,121.1,123.0,126.2] },
      // 通信費: 総務省CPI「通信」(2021春の携帯大幅値下げを反映)
      { id:"telecom",   name:"通信費",        series:[100.0,76.9,77.5,78.3,79.6,86.8,86.8,86.6,89.4,99.2,99.2] },
      // 広告: 総務省CPI「諸雑費」代替
      { id:"ad",        name:"広告・媒体費",  series:[100.0,101.5,103.1,103.8,104.5,106.1,105.4,110.5,107.4,112.0,108.5] },
      // 教材: 総務省CPI「教育」（推定）
      { id:"textbook",  name:"教材・コンテンツ", series:[100,101,102,103,104,105,106,107,108,109,110] },
      // 調達金利: 短期プライムレート(最頻値)の制度実値
      // 1.475%(2009-2024据置)→1.625%(2024-09)→1.875%(2025-03) / 1.475=100
      { id:"finance",   name:"調達金利コスト", series:[100,100,100,100,100,100,100,110.2,127.1,127.1,127.1] },
      // 医療材料: 総務省CPI「医療品・衛生材料」
      { id:"medsupply", name:"医療・衛生材料", series:[100.0,100.0,99.7,100.0,105.4,108.3,109.4,111.9,112.4,112.7,111.3] },
    ],
    // series に対応する月（全 inputs / actual 共通）
    months: ["2021-04","2021-10","2022-04","2022-10","2023-04","2023-10","2024-04","2024-10","2025-04","2025-10","2026-04"],
    ...
    categories: [
      {
        ...
        actual:[100.0,100.3,101.8,105.5,108.5,109.5,110.8,112.6,115.4,117.5,116.6],
      },
      {
        ...
        actual:[100.0,100.9,103.8,106.2,109.9,110.9,113.0,114.2,118.4,119.8,122.9],
      },
      {
        ...
        actual:[100.0,107.9,112.2,115.1,113.5,136.5,128.8,139.3,132.0,140.3,129.2],
      },
      {
        ...
        actual:[100.0,99.8,100.1,100.5,101.3,101.7,104.4,104.7,107.5,107.7,111.2],
      },
      {
        ...
        actual:[100.0,100.1,100.3,100.6,103.8,109.0,112.2,112.3,112.4,113.4,119.9],
      },
      {
        ...
        // 国交省「建設工事費デフレーター」住宅 — 資材高+人手不足（推定・唯一の非検証業態）
        actual:[100,108,122,128,126,127,130,133,136,138,140],
      },
      {
        ...
        actual:[100.0,100.1,101.3,101.3,104.9,105.0,107.5,107.5,110.4,110.5,113.1],
      },
      {
        ...
        actual:[100.0,100.1,100.2,100.7,102.0,102.8,103.5,104.1,104.8,105.4,106.1],
      },
      {
        ...
        actual:[100.0,100.6,103.3,107.1,107.6,110.1,113.3,114.5,115.1,115.9,131.7],
      },
      {
        ...
        actual:[100.0,103.1,106.1,92.7,114.7,132.3,136.2,142.4,144.6,154.4,151.2],
      },
      {
        ...
        actual:[100.0,101.8,104.0,108.2,112.7,117.5,117.6,121.6,125.3,129.4,129.7],
      },
      {
        ...
        actual:[100.0,100.6,100.6,102.1,102.1,102.1,102.6,103.8,103.8,103.8,103.8],
      },
      {
        ...
        actual:[100.0,101.5,103.1,103.8,104.5,106.1,105.4,110.5,107.4,112.0,108.5],
      },
    ],
  },
};
